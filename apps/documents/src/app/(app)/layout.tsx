...
import { RedirectToSignIn, SignedIn, SignedOut, UserButton, useAuth } from "@clerk/clerk-react";
import { Building2, FileText, Home, PenLine, Settings, Trash2 } from "lucide-react";
    ...
    <Tooltip>
      <TooltipTrigger
        render={
          <Link
            href={href}
            className="w-10 h-10 flex items-center justify-center rounded-lg transition-colors text-muted-foreground hover:bg-muted hover:text-muted-foreground"
          />
        }
      >
        {icon}
  ...
  return (
    <aside className="flex flex-col items-center w-[60px] border-r bg-card py-4">
      ...
      <nav className="flex flex-col items-center gap-1 flex-1">
        ...
        <SidebarLink href="/services" icon={<Building2 size={20} />} label="Services" />
