...
export default function DocumentDetail({ documentId, onClose }: DocumentDetailProps) {
  ...
  const [activeTab, setActiveTab] = useState<"details" | "signatures" | "activity">("details");
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {activeTab === "signatures" && (
              <div className="space-y-4">
                <SignatureStatus
                  key={signatureStatusKey}
                  documentId={doc.id}
                  readOnly={isLocked}
                  emptyState={
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center mb-4">
                        <PenLine size={24} strokeWidth={1.5} className="text-muted-foreground/30" />
                      </div>
                      <p className="text-sm font-medium text-foreground">No signatures requested</p>
                      <p className="text-xs text-muted-foreground mt-1 max-w-[220px]">
                        Send this document for signing and track progress here.
                      </p>
                      {!isLocked && (
