/**
 * Document operations — proxied to intake-api Lambda via HTTP.
...
async function apiFetch(path: string, init?: RequestInit) {
  const res = await fetch(`${INTAKE_API_URL}${path}`, {
    ...init,
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`intake-api ${path} returned ${res.status}: ${body}`);
  }
  return res.json();
}
