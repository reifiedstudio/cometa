...
export async function cleanupTrashedDocuments() {
  ...
  const trashedDocs = await db
    ...
    .where(and(eq(schema.documents.status, "rejected"), lt(schema.documents.updatedAt, cutoff)));
  ...
  console.log(
    `[cleanup] Found ${trashedDocs.length} rejected documents older than ${RETENTION_DAYS} days`,
  );
