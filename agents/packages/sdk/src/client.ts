...
export async function startSession(options: StartSessionOptions): Promise<SessionInfo> {
  ...
  await getAnthropicClient().beta.sessions.events.send(session.id, {
    events: [
      {
        type: "user.message",
...
export async function sendSessionEvent(sessionId: string, message: string): Promise<void> {
  await getAnthropicClient().beta.sessions.events.send(sessionId, {
    events: [
      {
        type: "user.message",
...
export async function streamSession(sessionId: string) {
  return getAnthropicClient().beta.sessions.events.stream(sessionId);
}
...
export async function getSession(sessionId: string) {
  return getAnthropicClient().beta.sessions.retrieve(sessionId);
}
...
export async function archiveSession(sessionId: string) {
  return getAnthropicClient().beta.sessions.archive(sessionId);
}
